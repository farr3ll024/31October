 ...
 *
 * The belt class creates a hashmap of variable size, which is the java representation
 * of a conveyor belt. The size is given to the belt class through the master class.
 * Item's can then be placed on the belt through the addItem method. 
 * The belt moves one space every tick, represented in the code by assigned each hashmap value to the next key (i.e 
 * the value at key = 0 is reassigned with key = 1. This is done from the end of the belt to the front in order to prevent
 * overwriting anything. Once the reassignment reaches the first spot in the hashmap, the program checks the Order class
 * to see if a new bin is ready to be added to the belt.
 */
