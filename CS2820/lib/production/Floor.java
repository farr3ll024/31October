 ...
 * NOTES: use Floor f = getFloor(); to set up the floor.
 * Shelf now is its own class (see Shelf.java)  Floor must still be called to retrieve Shelf data.
 * Floor class now has its own constructor.  To call everything, you'll need to set up
...
public class Floor 
{
	...
	{
		final Floor DEFAULT = new Floor(16, 16);
		return DEFAULT;
	}
    ...
    private void constructShelf() 
    {
        ...
        {
            this.shelf[i] = new Shelf(new Point());
        }
        ...
        for(int y = 2, j = 1; y <= (this.floor_Y - 3); y++, j++)
        {
        	...
        	for(int x = 3, i = 1; x <= (this.floor_X - 2); x++, i++)
        	{
        		...
        		{
        			this.shelf[s] = new Shelf(new Point(x,y));
        			this.s++;
        		}
        	}
        }
    ...
    public Shelf getShelf(int shelfNum)
    {
        ...
        {
            out.println("ERROR: must specify a valid shelf number between 0 and " + this.shelf.length);
            out.println("Please change the code to match.");
            return null;
        }
     ...
     */
    public Point getShelfPoint(int shelfNum)
    {
        ...
        return this.shelf[shelfNum - 1].getshelfBase();
    }
    ...
    public void printMap() 
    {
        ...
        for (int y = 0, q = 0; y <= floor_Y; y++) {
            for (int x = 0; x <= floor_X; x++) {
                ...
                else if (q < this.shelf.length && x == this.shelf[q].getshelfBase().getX() && y == this.shelf[q].getshelfBase().getY())
                {
                	out.print("| SH ");
                	q++;
                	continue;
    ...
    {
        out.println("Current Floor Dimensions: " + (this.floor_X + 1) + "x" + (this.floor_Y + 1));
        out.println("Number of Shelves: " + this.numberOfShelves());
        out.println("Charging Station: (" + ((int)this.getCharger().getX()) + ", " + ((int)this.getCharger().getY()) + ")");
        out.println("Receiving Dock: (" + ((int)this.getReceiving().getX()) + ", " + ((int)this.getReceiving().getY()) + ")");
        out.println("Shipping Dock: (" + ((int)this.getShipping().getX()) + ", " + ((int)this.getShipping().getY()) + ")");
        out.println("Picker Station: (" + ((int)this.getPicker().getX()) + ", " + ((int)this.getPicker().getY()) + ")");
        out.println("Packer Station: (" + ((int)this.getPacker().getX()) + ", " + ((int)this.getPacker().getY()) + ")");
        out.println("Belt: (" + ((int)this.getBelt()[0].getX())+", "+((int)this.getBelt()[0].getY())+")"+" to "
        +"(" + ((int)this.getBelt()[this.belt.length-1].getX())+", "+((int)this.getBelt()[this.belt.length-1].getY())+")");
        out.println("Shelf area: (" + ((int)this.getShelfPoint(1).getX())+", "+((int)this.getShelfPoint(1).getY())+")"+" to "
        +"(" + ((int)this.getShelfPoint(s).getX())+", "+((int)this.getShelfPoint(s).getY())+")");        
    }
