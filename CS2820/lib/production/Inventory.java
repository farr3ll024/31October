...
public class Inventory implements Clock, Document {
    ...
    public void data() {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                int amount = parseInt((String) item
                        .get(columnName[2]));
        ...
        if (checkExist(itemName, Qty) == true) {
            ...
            for (i = 0; i < inventory.size(); i++) {
                ...
                if (itemName.equals(removedItem.get("Name").toString())) {
                    int a = parseInt((String) removedItem.get("Amount"));
            ...
            out.println("Item " + itemName + " is not available.");
            for (int i = 0; i < inventory.size(); i++) {
                Map<String, Object> removedItem = new HashMap<>();
                removedItem = inventory.get(i);

                if (itemName.equals(removedItem.get("Name").toString())) {
                    out.println("Since item " + itemName + "'s amout is not enough, try less amount.");
                }
            }
        }
        outPutFile();
    ...
    public Map<Integer, Integer> readCapacity() {
        ...
        for (shelfId = 1; shelfId < inventory.size(); shelfId++) {
            ...
            for (i = 0; i < inventory.size(); i++) {
                ...
                int y = parseInt(x);
                if (shelfId == y) {
                    amount += parseInt(Item.get("Amount").toString());
                }
            }
            if (amount != 0) {
                countshelf += 1;
    ...
    public void outPutFile() {
        try (PrintWriter pw = new PrintWriter(new File("/Users/macbook_user/Desktop/OOP Project/List1.txt"))) {
    ...
    public boolean checkExist(String itemName, int Qty) {
        ...
        for (int i = 0; i < inventory.size(); i++) {
            ...
            newItem = inventory.get(i);
            if (itemName.equals(newItem.get("Name").toString())) {
                int a = parseInt((String) newItem.get("Amount"));
                //isInList = true;
                if (a >= Qty) {
                    isExist = true;
                    break;
